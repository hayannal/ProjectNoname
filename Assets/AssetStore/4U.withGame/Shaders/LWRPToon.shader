...
// This shader is effective on Unity2018.3 and later.
Shader "4U.withGame/LWRPToon"
{
	Properties
	{
		...
		[HideInInspector] _HueCB("HueChangeSRPBatching", Range(0,1)) = 0 //SRPBatching
	}
	SubShader
	{
		...
		Pass
		{
			...
			Varyings LitPassVertexToon(Attributes input)
			{
				...
				VertexPositionInputs vertexInput = GetVertexPositionInputs(input.pos.xyz);	//2018.3
				VertexNormalInputs normalInput = GetVertexNormalInputs(input.normal);// , input.tangentOS);//2018.3
			...
			half4 LitPassFragmentToon(Varyings input) : SV_Target
			{
				...
				diffuse = hueChange(diffuse, _HueCB * 9); //SRPBatching


				half3 normal = NormalizeNormalPerPixel(input.normal);	//2018.3
				half3 bakedGI = SAMPLE_GI(input.lightmapUV, input.vertexSH, normal);
				
				Light mainLight = GetMainLight(input.shadowCoord); //()2018.2
				...
				int pixelLightCount = GetAdditionalLightsCount(); //GetPixelLightCount();	//2018.2
				for (int i = 0; i < pixelLightCount; ++i)
				{
					Light light = GetAdditionalLight(i, input.posWS);//GetLight(i, input.posWS);//2018.2
