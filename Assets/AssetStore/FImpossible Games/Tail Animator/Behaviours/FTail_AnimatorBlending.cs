...
namespace FIMSpace.FTail
{
    ...
    /// </summary>
    [AddComponentMenu("FImpossible Games/Tail Animator/FTail Animator Blending")]
        ...
        public override void CalculateOffsets()
        {
            ...
            if (BlendToOriginal >= 1f)
            {
                ...
                if (smoothChainBlend < 1f)
                {
                    ...
                    {
                        proceduralPoints[i].Position = Vector3.Lerp(proceduralPoints[i].Position, TailTransforms[i].position, smoothChainBlend);
                        proceduralPoints[i].Rotation = Quaternion.Lerp(proceduralPoints[i].Rotation, TailTransforms[i].rotation, smoothChainBlend);
                    ...
                    for (int i = 0; i < proceduralPoints.Count; i++)
                    {
                        proceduralPoints[i].Position = TailTransforms[i].position;
                        proceduralPoints[i].Rotation = TailTransforms[i].rotation;
                    }
            ...
            if (BlendToOriginal > 0 || BlendChainValue > 0f)
            {
                for (int i = 0; i < TailTransforms.Count; i++)
                {
                    if ((float)(i) / (float)TailTransforms.Count <= BlendChainValue)
                    {
                        ...
                        proceduralPoints[i].Position = Vector3.Lerp(proceduralPoints[i].Position, TailTransforms[i].position, BlendToOriginal * blendValue);
                        proceduralPoints[i].Rotation = Quaternion.Lerp(proceduralPoints[i].Rotation, TailTransforms[i].rotation, BlendToOriginal * blendValue);
                    }
