	...
	SubShader{
		...
		Pass {
			...
			fixed4 frag(v2f i) : COLOR {
				...
				fixed3 diffuseReflection = UNITY_LIGHTMODEL_AMBIENT.rgb+_LightColor0.rgb * tmpDot;

				fixed3 specularReflection = fixed3(0.0h, 0.0h, 0.0h);

				if (tmpDot > 0.0h)
				{
					specularReflection = _LightColor0.rgb * pow(max(0.0h, dot(reflect(-lightDirection, normalDirection), viewDirection)), _Shininess);
				}
