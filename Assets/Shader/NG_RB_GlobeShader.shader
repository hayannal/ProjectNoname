Shader "FrameworkNG/GlobeShader" {
	...
	SubShader{
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			fixed4 c = tex2D(_MainTex, uv1) * tex2D(_MainTex2, uv2)  * tex2D(_MainTex3, uv3) * _Color;
			fixed a = tex2D(_MaskTex, uvmasx).r;
			fixed x = tex2D(_HotlineTex, uvx).r * _HotlineColor;
			...
			c = c + ((c * x.r * 5) + x.r) * x.r * 2 * _HotlineColor.a;
			o.Albedo = c.rgb;
			o.Alpha = c.a;
		}
		ENDCG
